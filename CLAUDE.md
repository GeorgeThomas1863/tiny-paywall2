...
- Data model (full definition in SPEC.md §2–3): `users`, `sessions`, `articles`, `purchases`, `ledger`, `payout_requests`. All money is integer cents; balances change only inside Mongo transactions in `money/operations.py` (SPEC §2.4). Indexes are created idempotently at startup by `ensure_indexes()` — currently users.email (unique), users.display_name (unique, case-insensitive collation), sessions.expires_at (TTL); the rest land per SPEC phases. The Motor client is `tz_aware=True` — datetimes read back from Mongo are timezone-aware UTC.

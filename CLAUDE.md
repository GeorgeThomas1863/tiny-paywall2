...
- Votes (SPEC §4.4 `routes/votes.py`): purchasers up/downvote articles Reddit-style via `PUT /articles/{id}/vote` (`value: 1 | -1 | 0`, 0 clears). The vote lives as an optional `vote` field on the buyer's `purchases` doc — the purchase row is the eligibility check (no purchase → 403; authors/admins excluded automatically). Scores are aggregated at read time in `routes/articles.py` (`aggregate_scores`); `score` is on every teaser/detail, `my_vote` (1/-1/0, or null for non-buyers) on detail only.
